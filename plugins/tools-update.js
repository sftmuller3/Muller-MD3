...
cmd({ 
  ...
  try { 
    await conn.sendMessage(from, { text: '📡 Please wait... Checking for Muller updates...' }, { quoted: mek });
    
    if (!fs.existsSync('./.git')) { 
      console.log("Initializing git repository..."); 
      execSync('git init'); 
      execSync('git remote add origin https://github.com/Mrabdul13/Muller-MD.git'); 
    } else { 
    ...
    if (localCommit === originCommit) { 
      await conn.sendMessage(from, { text: '*✅ Muller-MD Bot is already up to date!*' }, { quoted: mek });
    } else { 
      ...
      await conn.sendMessage(from, { text: '*✅ Muller-MD updated successfully!*' }, { quoted: mek });
    }
  } catch (error) { 
    console.error(error); 
